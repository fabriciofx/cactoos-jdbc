    ...
    public Cacheable(final Query query) {
        this.scalar = new Sticky<>(
            () -> {
                ...
                stmt.accept(new CacheableShuttle(cacheable, tables, columns));
                if (!cacheable.get()) {
                    return false;
                }
                for (final String table : tables) {
                    schema.add(table, new CacheableTable(table, columns));
                }
                ...
                new CacheableVisitor(cacheable).go(relational);
